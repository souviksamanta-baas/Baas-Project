...
The owner picks a contact → phone is normalized to E.164 → stored on `organization_invites.invited_phone_e164`. The invited staff member must verify the **same** number when accepting the QR invite.

## CRM / clients

Reuse the same contact picker when creating or editing CRM contacts. Normalize Argentina formats (`011…`, `+5411…`, `+54911…`) before save.

## Platforms

- **iOS / Android:** `NSContactsUsageDescription` / `READ_CONTACTS` — add copy in app config when publishing to stores
